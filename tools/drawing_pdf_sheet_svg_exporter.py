...
Optional crop is applied as a PDF page viewport clip. This may translate the
display coordinate origin, but it does not redraw or semantically mutate the
source geometry.
"""
...
def export_sheet(
    ...
    if crop is None:
        ...
        display_translation = [0.0, 0.0]
        crop_mode = "NONE"
    else:
        clipped = fitz.open()
        clipped_page = clipped.new_page(width=crop_rect.width, height=crop_rect.height)
        clipped_page.show_pdf_page(clipped_page.rect, doc, page_index, clip=crop_rect)
        svg = clipped_page.get_svg_image(matrix=fitz.Matrix(1, 1), text_as_path=False)
        display_translation = [-crop_rect.x0, -crop_rect.y0]
        crop_mode = "PAGE_CLIP"
