...
<path id="src-1" d="M 20 0 L 30 0" stroke="#000" fill="#fff"/>
</svg>'''
...
class SelectiveEditorTests(unittest.TestCase):
    def test_verified_suppression_preserves_geometry(self):
        ...
        self.assertIn('visibility="hidden"',result["svg"])
        self.assertIn('opacity="0"',result["svg"])
        self.assertIn('stroke-opacity="0"',result["svg"])
        self.assertIn('fill-opacity="0"',result["svg"])
        self.assertIn('d="M 20 0 L 30 0"',result["svg"])
