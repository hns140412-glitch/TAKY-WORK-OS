...
const base={
  artifact_class:'FINAL',operations:['VECTOR_STYLE_LAYER'],gates:pass,
  executor_type:'AUTHORIZED_DRAWING_ENGINE',route_owner:'DRAWING_FINALIZATION_ORCHESTRATOR',
  execution_receipt:receipt,validation_bundle_id:'VB-001',source_digest:'sha256:source',artifact_digest:'sha256:artifact'
};
assert.equal(g.evaluate(base).decision,'SHOW');
assert.equal(g.evaluate({...base,executor_type:'PYTHON'}).decision,'HOLD');
assert.equal(g.evaluate({...base,executor_type:'REPORTLAB'}).decision,'HOLD');
assert.equal(g.evaluate({...base,executor_type:'GENERIC_HTML'}).decision,'HOLD');
assert.equal(g.evaluate({...base,route_owner:'LOCAL_SCRIPT'}).decision,'HOLD');
